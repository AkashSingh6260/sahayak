...
const Broadcast = () => {
  ...
  const confirmBroadcast = async () => {
    try {
      ...
      toast.success("📢 Request broadcasted to nearby professionals!");
      ...
      <div className="relative h-[300px] w-full overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-black/70 via-black/50 to-indigo-900/40 flex items-center justify-center">
          <div className="text-white text-center space-y-2">
            ...
            <h1 className="text-4xl font-bold capitalize mt-3">{serviceType} Service</h1>
        ...
        <div className="bg-white/95 backdrop-blur-xl shadow-[0_20px_60px_-10px_rgba(0,0,0,0.25)] rounded-3xl p-8 w-full max-w-xl border border-indigo-100">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-2 bg-gradient-to-r from-indigo-50 to-blue-50 text-indigo-700 px-4 py-2 rounded-full border">
              <Wrench size={16} />
              <span className="font-semibold capitalize">{serviceType?.replace("_", " ") || serviceType}</span>
            </div>
          ...
          <div className="mt-6 rounded-2xl border border-indigo-100 bg-gradient-to-br from-white to-indigo-50 p-5">
            ...
            {location ? (
              ...
              <p className="text-sm text-slate-500 mt-1">Location not shared</p>
            )}
            <button
          ...
          <div className="mt-6 rounded-2xl border border-blue-100 bg-gradient-to-br from-blue-50 to-indigo-50 p-5">
            ...
            <div className="flex justify-between text-sm text-slate-700 py-2 border-b border-blue-100">
              <span>
                Service Charge
                {serviceRate.type === "hourly" && <span className="text-xs text-slate-400 ml-1">(per hr)</span>}
              </span>
              <span className="font-semibold text-slate-900">
                {serviceRate.type === "fixed" ? `₹${serviceRate.price} fixed` : `₹${serviceRate.rate}/hr`}
              </span>
            </div>

            {/* Distance charges table */}
            <div className="mt-2">
              <p className="text-xs font-medium text-slate-500 mb-1.5">+ Distance Charge</p>
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-3xl p-7 w-full max-w-md shadow-2xl border">
            <h3 className="text-xl font-semibold mb-2">Confirm Broadcast</h3>
            <p className="text-sm text-slate-600 mb-5">
              This request will be sent to nearby <b className="capitalize">{serviceType}</b> professionals.
            </p>
      ...
      {waiting && (
        <div className="fixed inset-0 bg-black/70 z-[999] flex items-center justify-center">
          <div className="bg-white rounded-3xl p-8 text-center shadow-2xl w-full max-w-sm mx-4">
            ...
            <h3 className="text-lg font-semibold mb-1">Finding a {serviceType}...</h3>
            <p className="text-sm text-gray-500 mb-6">Please wait while nearby professionals respond</p>
            <button
      ...
      {accepted && (
        <div className="fixed inset-0 bg-black/70 z-[999] flex items-center justify-center">
          <div className="bg-white rounded-3xl p-8 text-center shadow-2xl w-full max-w-sm mx-4">
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-1">Request Accepted!</h3>
            {providerInfo?.providerName && (
              <p className="text-slate-600 text-sm mt-2">
                <b>{providerInfo.providerName}</b> is on the way.
                {providerInfo.providerPhone && (
                  <span> Call: <a href={`tel:${providerInfo.providerPhone}`} className="text-indigo-600 font-bold">{providerInfo.providerPhone}</a></span>
                )}
              </p>
            )}
            <button
