...
export const getNearbyRequests = async (req, res) => {
  try {
    ...
    // Sort SOS requests to the absolute top, retaining newest-first order within type
    uniqueRequests.sort((a, b) => {
      const isASos = a.requestType === "SOS";
      const isBSos = b.requestType === "SOS";
      if (isASos && !isBSos) return -1;
      if (!isASos && isBSos) return 1;
      return 0; // Leave unchanged order for same types
    });
...
export const createSOSRequest = async (req, res) => {
  try {
    const { problemDescription, location, issueType } = req.body;

    if (!problemDescription || !location?.lat || !location?.lng) {
    ...
    // Check if user is blocked from making SOS requests due to false alarms
    if (user.sosBlockedUntil && new Date() < user.sosBlockedUntil) {
      return res.status(403).json({ 
        message: "You are blocked from making SOS requests due to past false alarms." 
      });
    }

    const service = await ServiceRequest.create({
      customerId: req.userId,
      serviceType: "sos", // General category, can be superseded by actual issue if provided
      problemDescription,
      ...
      requestType: "SOS",
      severity: "CRITICAL",
      expiresAt: new Date(Date.now() + 30 * 60 * 1000), // Expiry 30 minutes for SOS
    });

    // Increment user SOS count
    user.sosCount = (user.sosCount || 0) + 1;
    await user.save();

    // Broadcast to ALL nearby approved & available partners within 25km
    const partners = await Partner.find({
      isAvailable: true,
      status: "approved",
      location: { $exists: true },
    ...
    // Record DB notifications
    await Notification.insertMany(
      nearbyPartners.map((p) => ({
        user: p.user,
        message: `🚨 EMERGENCY: SOS Request near your location!`,
        type: "sos_alert",
        relatedId: service._id,
        status: "unread",
      }))
    ...
    // Blast real-time WebSocket SOS alarms to all nearby active providers
    nearbyPartners.forEach((p) => {
      notifyUser(p.user.toString(), {
        type: "sos_alarm",
        message: `🚨 SECONDS COUNT: EMERGENCY SOS REQUEST NEAR YOU!`,
        data: {
          requestId: service._id,
          requestType: "SOS",
          problemDescription,
    ...
    res.status(201).json({
      message: "SOS Alert Broadcasted to all nearby service providers",
      service,
      notifiedPartners: nearbyPartners.length,
    });
  } catch (error) {
...
/* =======================
   SUBMIT SOS FEEDBACK (Provider Side)
======================= */
export const submitSOSFeedback = async (req, res) => {
  try {
    const { requestId, feedback } = req.body; 

    const request = await ServiceRequest.findById(requestId);
    if (!request) return res.status(404).json({ message: "SOS Request not found" });

    if (request.requestType !== "SOS") {
      return res.status(400).json({ message: "Not an SOS request" });
    }

    request.providerFeedback = feedback;
    await request.save();

    // Penalize the user for fake or non-emergency use of the SOS button
    const customer = await User.findById(request.customerId);
    if (customer && (feedback === "fake" || feedback === "normal")) {
      customer.falseSosCount = (customer.falseSosCount || 0) + 1;
      customer.trustScore = Math.max(0, (customer.trustScore || 100) - 20); // Deterrent penalty
    
      if (customer.trustScore < 60) {
        customer.sosBlockedUntil = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000); // 7 days ban
      }
      await customer.save();
    }

    res.json({ message: "SOS feedback submitted successfully", trustScoreUpdated: true });
  } catch (error) {
    console.error("SOS FEEDBACK ERROR:", error);
    res.status(500).json({ message: "Server error" });
  }
};
